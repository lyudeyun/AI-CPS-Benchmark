...
%% setup parameters
Ts = 0.01;           % simulation duration in sec
T = 30;             % sample time
fuel_inj_tol=1.0;
...
criticNet = [
    ...
    fullyConnectedLayer(300,'Name', 'fc_in')
    reluLayer('Name', 'relu1')
    fullyConnectedLayer(400,'Name', 'fc_in2')
    reluLayer('Name', 'relu2')
    fullyConnectedLayer(400,'Name', 'fc_in3')
...
inPath = [ 
    imageInputLayer([obsInfo.Dimension 1], 'Normalization','none','Name','state')
    fullyConnectedLayer(300,'Name', 'ip_fc')  % 10 by 1 output
    reluLayer('Name', 'ip_relu1')             % nonlinearity
    fullyConnectedLayer(400,'Name', 'ip_fc1')  
...
actorOpts = rlRepresentationOptions('LearnRate',1e-3,'GradientThreshold',10);
